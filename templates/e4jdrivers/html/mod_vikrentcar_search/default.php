...
<div class="<?php echo $params->get('moduleclass_sfx'); ?> center">
	...
	jQuery.ajax({
		...
		if(where == "pickup" && vrcmod_location_change === false) {
			...
			vrcmod_location_change = false;
					...
					$vrloc.="<div class=\"vrcsfentrycont vrcsfentrysel-pk-place\"><label>".JText::_('VRMPPLACE')."</label><div class=\"vrcsfentryselect\"><select name=\"place\" id=\"modplace\"".$onchangeplaces.">";
...
jQuery(function(){
	...
	jQuery('#releasedatemod').datepicker( 'option', jQuery.datepicker.regional[ 'vikrentcarmod' ] );
		...
		echo "<div class=\"vrcsfentrycont vrcsfentrysel-pk-date\"><div class=\"vrcsfentrylabsel\"><label>" . JText :: _('VRMPICKUPCAR') . "</label><div class=\"vrcsfentrydate\"><input type=\"text\" name=\"pickupdate\" id=\"pickupdatemod\" size=\"10\" autocomplete=\"off\"/></div></div><div class=\"vrcsfentrytime\"><label>" . JText :: _('VRMALLE') . "</label><span id=\"vrcmodselph\"><select name=\"pickuph\">" . $hours . "</select></span><span class=\"vrctimesep\">:</span><span id=\"vrcmodselpm\"><select name=\"pickupm\">" . $minutes . "</select></span></div></div>\n";
		echo "<div class=\"vrcsfentrycont vrcsfentrysel-rt-date\"><div class=\"vrcsfentrylabsel\"><label>" . JText :: _('VRMRETURNCAR') . "</label><div class=\"vrcsfentrydate\"><input type=\"text\" name=\"releasedate\" id=\"releasedatemod\" size=\"10\" autocomplete=\"off\"/></div></div><div class=\"vrcsfentrytime\"><label>" . JText :: _('VRMALLEDROP') . "</label><span id=\"vrcmodseldh\"><select name=\"releaseh\">" . $hoursret . "</select></span><span class=\"vrctimesep\">:</span><span id=\"vrcmodseldm\"><select name=\"releasem\">" . $minutesret . "</select></span></div></div>";
	}else {
		echo "<div class=\"vrcsfentrycont vrcsfentrysel-pk-date\"><div class=\"vrcsfentrylabsel\"><label>".JText::_('VRMPICKUPCAR')."</label><div class=\"vrcsfentrydate\">".JHTML::_('calendar', '', 'pickupdate', 'pickupdatemod', $dateformat, array('class'=>'', 'size'=>'9',  'maxlength'=>'19'))."</div></div><div class=\"vrcsfentrytime\"><label>".JText::_('VRMALLE')."</label><span id=\"vrcmodselph\"><select name=\"pickuph\">".$hours."</select></span><span class=\"vrctimesep\">:</span><span id=\"vrcmodselpm\"><select name=\"pickupm\">".$minutes."</select></span></div></div>\n";
    ...
    if(@is_array($places)) {
    	$vrlocreturn="";
    	$vrlocreturn.="<div class=\"vrcsfentrycont vrcsfentrysel-rt-place\"><label>".JText::_('VRMPLACERET')."</label><div class=\"vrcsfentryselect\"><select name=\"returnplace\" id=\"modreturnplace\"".(strlen($onchangeplacesdrop) > 0 ? $onchangeplacesdrop : "").">";
		foreach($places as $pla){
    	...
    	if ($dbo->getNumRows() == 1) {
    		...
    		if(intval($sc[0]['setting'])==1){
				...
				if ($dbo->getNumRows() > 0) {
					...
					$vrcats.="<div class=\"vrcsfentrycont vrcsfentrysel-cat\"><label>".JText::_('VRMCARCAT')."</label><div class=\"vrcsfentryselect vrcsfentryselcat\"><select name=\"categories\">";
					$vrcats.="<option value=\"all\">".JText::_('VRMALLCAT')."</option>\n";
					foreach($categories as $cat){
						$vrcats.="<option value=\"".$cat['id']."\">".JText::_($cat['name'])."</option>\n";
					}
    ...
    ?>
    <div class="vrcsfentrycont vrcsfentrysel-sbm"><div class="vrcsfentrysubmit"><input type="submit" name="search" class="btn vrcsearch" value="<?php echo (strlen($params->get('srchbtntext')) > 0 ? $params->get('srchbtntext') : JText::_('SEARCHD')); ?>"/></div></div>
    </form>
</div>
</div>
